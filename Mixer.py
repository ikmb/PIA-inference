...
def parse_user_inputs()->Dict[str,str]:
    ...
    parser.add_argument('--input_genetic_table',help='A table of three columns, containing first, the peptide sequence, second an identifier that contains further information\
 about the peptide. This field MUST Starts with the protein uniprot IDs. The third columns contain phenotypic information, e.g. Reference, control, etc.',
    ...
    if args.input_genetic_table=='?':
        ...
        sys.exit(-1)
    if not os.path.exists(args.input_genetic_table):
        sys.stderr.write(f'ERROR: {time.ctime()}: I {sys.argv[0]} encounterred the following problem: The path to the input genetic table: {args.input_genetic_table} has not been provided.')
        sys.exit(-1)
    try:
        dev_read=pd.read_csv(args.input_genetic_table,nrows=10,sep='\t')
    except Exception as exp:
        sys.stderr.write(f'ERROR: {time.ctime()}: I {sys.argv[0]} encounterred the following problem: Reading the input genetic table provided at: {args.input_genetic_table}\
             risen the following exception: {str(exp)}')
        sys.exit(-1)
    if dev_read.shape[1] != 3:
        sys.stderr.write(f'ERROR: {time.ctime()}: I {sys.argv[0]} encounterred the following problem: Incorrect table provided at: {args.input_genetic_table}\
             expect number of columns is 3 columns. However, your input has : {dev_read.shape[1]} columns')
        sys.exit(-1)
    # Load the list of alleles
    #-------------------------
    if args.standardized_allele_name=='?':
        sys.stderr.write(f'ERROR: {time.ctime()}: I {sys.argv[0]} encounterred the following problem: The list of alleles has not been provided.\n')
        sys.exit(-1)
    if not os.path.exists(args.standardized_allele_name):
        sys.stderr.write(f'ERROR: {time.ctime()}: I {sys.argv[0]} encounterred the following problem: The path to the list of alleles: {args.input_genetic_table} has not been provided.')
        sys.exit(-1)
    ## check the output is path is valid
    #-----------------------------------
    if not os.path.exists(os.path.dirname(args.results_path)):
        sys.stderr.write(f'ERROR: {time.ctime()}: I {sys.argv[0]} encounterred the following problem: The base to write the results:\
    ...
    #--------------------
    return {
        'input_genetic_table':args.input_genetic_table,
...
def mix_inputs(path2pep:str, path2alleles:str)->pd.DataFrame:
    ...
    result_df=pd.DataFrame(columns=peptides.columns+['allele'])
    # repeat the table for each input allele
    for allele in tqdm(alleles.iloc[:,0].to_list()):
...
if __name__=='__main__':
    ...
    input2predictions=mix_inputs(user_arguments['input_genetic_table'],user_arguments['standardized_allele_name'])
    input2predictions.to_csv(user_arguments['results_path'])
